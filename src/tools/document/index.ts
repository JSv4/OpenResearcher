...
const WORKING_DIRECTORY = path.join(process.cwd(), "workspace");
...
export const readDocumentTool = tool(
  ...
  export const createOutlineTool = tool(
    async ({ sections, filename }) => {
        const outline = sections.map(section => `# ${section}\n\n`).join("");
        const filePath = path.join(WORKING_DIRECTORY, filename);
        await fs.writeFile(filePath, outline);
        return `Created outline in ${filename}`;
    },
    {
      name: "create_outline",
      description: "Create a document outline in markdown format",
      schema: z.object({
